...
const getTimeRange = (start_date, end_date) => {
    ...
    if (start_date.includes('-7d/d')) {
        start_date = moment(new Date() - 7 * 24 * 60 * 60 * 1000).utc().format('YYYY-MM-DD');
    } else if (start_date.includes('-30d/d')) {
        start_date = moment(new Date() - 30 * 24 * 60 * 60 * 1000).utc().format('YYYY-MM-DD');
    ...
    if (end_date.includes('now/d')) end_date = moment().utc().format("YYYY-MM-DD HH:mm:ss");

...
const { Province, District, Ward, ShippingAddress, sequelize } = require('../models');
...
const addNewShippingAddress = async (req, res, next) => {
    try {
        ...
        }).then(() => res.status(200).send("success")).catch((e) => {
            if (e.message === "Thêm tối đa 5 địa chỉ") return next({ code: 400, msg: "Thêm tối đa 5 địa chỉ" })
            next({})
        })
        return;
    } catch (error) {

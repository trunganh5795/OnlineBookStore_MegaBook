...
const getProduct = async (req, res, next) => {
    try {
        ...
        const result = await Book.findByPk(+id, {
            attributes: { exclude: ['sku', 'title2', 'desc2', 'bestbook', 'createdAt', 'updatedAt'] }
        });
        // Trả về
...
const getAllProducts = async (req, res, next) => {
    try {
        let { page, perPage, filterField, filterValue, sorterField, sorterValue } = req.query;
        if (!page) page = 1;
...
const getSearchProducts = async (req, res, next) => {
    try {
        ...
        const result = await Book.findAndCountAll({
            ...
            limit: +perPage,
            where: {
                name: sequelize.where(sequelize.fn('LOWER', sequelize.col(`Book.title`)), { [Op.like]: `%${value.toLowerCase()}%` }),
                status: 'active',
            },
        });
...
const getAllProductsByType = async (req, res, next) => {
    try {
        ...
        switch (+sortType) {
            ...
            case 4:
                sortType = {}
                if (price_to < price_from) return next({ code: 400, msg: "Xảy ra lỗi" })
                whereClause.price = {
        ...
        let products = await Book.findAndCountAll({
            ...
            where: whereClause,
            order: order,
            attributes: ["bookid", "title", "stars", 'author', 'publicOfYear', 'comment', 'img', 'instock', 'price', 'discount', 'enable_discount', 'start_time', 'end_time', 'total_rate']
        })
...
const getRecommendProducts = async (req, res, next) => {
    try {
        ...
        let result = await Book.findAll({
            ...
            attributes: {
                exclude: ['QRcodeImg', 'category', 'width', 'height', 'comment', 'createdAt', 'updatedAt', 'bookId']
...
const getTopSell = async (req, res, next) => {
    try {
        let result = await sequelize.query("select `bookId` as `BookId`,`title`,`price`,`instock`,`stars`,`img`,`discount`, top_sell.sold as `sold` from books as b INNER JOIN ( select `book_id`, sum(`quantity`) as `sold` from order_details group by book_id order by sum(`quantity`) desc limit 10) as top_sell on b.bookId = top_sell.book_id where `status` = 'active';", { type: QueryTypes.SELECT })
...
const getBestBook = async (req, res, next) => {
    try {
        let data = await Book.findOne({
            ...
            attributes: ['bookId', 'title', 'author', 'img', 'category', 'price', 'desc2', 'discount', 'start_time', 'end_time', 'enable_discount', 'instock']
        })

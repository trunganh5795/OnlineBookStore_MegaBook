...
const getUser = async (req, res, next) => {
  try {
    ...
    const infoUser = await User.findByPk(+id, {
      attributes: ["id", "dateOfBirth", "email", "phone_no", "address", "name", "gender", "img","balance"]
    })
...
const putUpdateUser = async (req, res, next) => {
  try {
    ...
    if (avatar) {
      imgURL = await uploadProductAvt(`${userDetail.name}-${userDetail.id}`, avatar, '/user-avatar')
    }

    if (userDetail) {
      const response = await userDetail.update({ name, gender, dateOfBirth, img: imgURL });
      if (response) {

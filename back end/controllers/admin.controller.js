const { Notification, Predict_cancellation, Book, sequelize, Order, User, Voucher, Percentage_discount, amount_discount } = require('../models');
const axios = require('axios').default;
const QRCode = require('qrcode');
const { Op, QueryTypes } = require("sequelize");
const { cloudinary } = require('../config/cloudinary.config');
const io = require('../config/socket.io.config');
const { findSocketIdInConnectedUserList, removeVietnameseTones } = require('../helpers');
...
const genQrCode = async (qrCodeURL) => {
  return QRCode.toDataURL(qrCodeURL, { errorCorrectionLevel: 'H', version: 5, mode: 'alphanumeric' })
}
...
const adminLogin = async (req, res, next) => {
  try {
    ...
    if (userDetail) {
      let isMatchPassword = bcrypt.compareSync(password.toString(), userDetail.password)
      if (!isMatchPassword) {
...
const addProduct = async (req, res, next) => {
  try {
    ...
    if (!(avatar && title && price && instock && width && height)) return next({ code: 400, msg: "Bad request" })
    ...
    clientRecombee.send(new rqs.SetItemValues(newProduct.bookId, {
    ...
    let qrCodeURL = process.env.FONT_END_PUBLIC_URL ? process.env.FONT_END_PUBLIC_URL + `/qrcode/${newProduct.BookId}` : `http://localhost:3000/qrcode/${newProduct.BookId}`
    let base64QRcode = await genQrCode(qrCodeURL);
    let qrCodeImageURL = await uploadQrCodeIamge(newProduct.title, base64QRcode);
...
const updateProduct = async (req, res, next) => {
  try {
    ...
    if (!rest.avatar) {
      book.set({ ...rest, img: book.img, start_time: time[0], end_time: time[1] })
    } else {
      const avtUrl = await uploadProductAvt(rest.title, rest.avatar);
      book.set({ ...rest, img: avtUrl, start_time: time[0], end_time: time[1] })
    }
...
const getCustomerListByValue = async (req, res, next) => {
  try {
    ...
    if (option) whereClause[option] = sequelize.where(sequelize.fn('LOWER', sequelize.col(`User.${option}`)), { [Op.like]: `%${value.toLowerCase()}%` });
...
const getOrderList = async (req, res, next) => {
  try {
    ...
    let { page, perPage, filterStatusValue, filterPaymentValue, sorterField, sorterValue } = req.query;
    if (!page) page = 1;
  ...
  try {
    ...
    if (order) {
      Promise.all([
        ...
        let socketId = findSocketIdInConnectedUserList(io.connectedUser, order.user_id)
...
const getOrderListByValue = async (req, res, next) => {
  try {
    ...
    let { value, option, page, perPage, filterStatusValue, filterPaymentValue, sorterField, sorterValue } = req.query;
    if (!page) page = 1;
    if (!perPage) perPage = 8;
    const nSkip = (parseInt(page) - 1) * perPage;
    ...
    if (+option === 0) return getOrderList(req, res, next)
      ...
      let result = await Order.findAndCountAll({
        ...
        include: {
          model: User,
          where: {
            name: sequelize.where(sequelize.fn('LOWER', sequelize.col(`name`)), { [Op.like]: `%${value.toLowerCase()}%` })
          },
          attributes: ['name'],
          as: 'user'
        },
...
const getAllVouchers = async (req, res, next) => {
  try {
    ...
    if (stateFilterValue) {
      if (stateFilterValue === 1) { //voucher đang diễn ra
      ...
      } else if (stateFilterValue === 2) { //voucher sắp diễn ra
        whereClause.start_time = {
          [Op.gte]: new Date()
        };
        whereClause.end_time = {
...
const getVoucherByCategory = async (req, res, next) => {
  try {
    ...
    let voucherList = await Voucher.findAndCountAll({
      ...
      where: {
        delete: 0,
        apply: { //thuộc tính apply là null or 1
          [Op.or]: voucherType
        }
      }
    });
...
const adminSearchProduct = async (req, res, next) => {
  try {
    ...
    let { value, page, perPage, option, filterField, filterValue, sorterField, sorterValue } = req.query;
    if (!page) page = 1;
    if (!perPage) perPage = 10;
    let query = {
      ...
      body: {
        ...
        _source: ["bookId", "title", "category", "price", "instock", "img", "author", "width", "height", "QRcodeImg", "publicOfYear", "publisher"],
        aggs: {
          count: {
            value_count: {
              field: 'bookId'
    ...
    res.status(200).send({ count: result.body.aggregations.count.value, rows: result.body.hits.hits })
  } catch (error) {
    next({})
  }
...
const calculateCancellationRate = async (orderid, userid, next) => {
  try {
    ...
    let ratio = 0
    if (rate.length) {
      ratio = rate.reduce((count, item, index) => item.status === "5" ? count + 1 : count, 0) / rate.length;
    }
    let order = await sequelize.query("select total,shipping,voucher,sum(quantity) as quantity  from book_store.orders left join book_store.order_details on orders.id = order_details.order_id where id = $id;", {
    ...
    if (order[0].quantity) {
      let predict = await axios.get('http://localhost:5000/getratio', {
        params: {
...
const updateNumOfContact = async (req, res, next) => {
  try {
    ...
    await Predict_cancellation.increment({ contact_time: 1 }, { where: { order_id: orderId } });

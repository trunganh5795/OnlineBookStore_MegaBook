...
const countOrderAndTotal = async (req, res, next) => {
    try {
        if (req.user.role !== 'admin') return next({ msg: 'Bạn không có quyền truy cập', code: 401 })
...
const getTop5BestSell = async (req, res, next) => {
    try {
        ...
        let idList = result1.body.aggregations.topSellById.buckets.map(item => item.key)
        // 
        const result = await client.search({
            index: 'books',
                ...
                sort: [
                    {
                        _script: {
                            type: "number",
                            script: {
                                inline: "if(doc['bookId'] != null){for(int i=0;i<params.sortOrder.length;i++){if(doc['bookId'].value==params.sortOrder[i]){return i}}}else{return 0;}",
                                params: {
                                    sortOrder: idList
                                },
                            },
        ...
        if (result.body.hits) {
            for (let i = 0; i < result.body.hits.hits.length; i++) {
                result.body.hits.hits[i].numberOfSell = result1.body.aggregations.topSellById.buckets[i].quantity.value
            }
            res.status(200).send(result.body.hits.hits);
        } else {
            res.status(200).send([]);
...
const getTop5Total = async (req, res, next) => {
    try {
        ...
        //     ]
        // })
        let data = await sequelize.query("SELECT `Order_detail`.`book_id`, `Order_detail`.`discount`, SUM(`Order_detail`.quantity * (1-`Order_detail`.discount/100)* `product`.price) AS `total`, `product`.`bookId` AS `bookId`, `product`.`title` AS `title` FROM `order_details` AS `Order_detail` LEFT OUTER JOIN `books` AS `product` ON `Order_detail`.`book_id` = `product`.`bookId` WHERE (`Order_detail`.`createdAt` >= :start AND `Order_detail`.`createdAt` <= :end) GROUP BY `book_id` ORDER BY total DESC LIMIT 5;",
        {
...
const getTop5LocationSell = async (req, res, next) => {
    try {
        ...
        const result = await client.search({
            index: 'orders',
            filter_path: ['aggregations.topLocation.buckets', 'aggregations.topLocation.buckets.hits.hits.hits'],
            // _source: false,
            body: {
                query: {
                    range: {
...
const getTop5SellByCategory = async (req, res, next) => {
    try {
        ...
        let { start_date, end_date } = req.query
        let [start, end] = getTimeRange(start_date, end_date);
        let result = await sequelize.query("select `categories`.name , sum(quantity )AS quantity from order_details left join (books left join categories on `books`.category = `categories`.id) on `order_details`.book_id = `books`.bookId where(`order_details`.createdAt >= :start AND `order_details`.createdAt <= :end) group by `books`.category order by quantity desc;",
            {
                type: QueryTypes.SELECT,
                replacements: { start, end }
            })
...
const numOfOrderByStatus = async (req, res, next) => {
    try {
        ...
        let { start_date, end_date } = req.query
        let [start, end] = getTimeRange(start_date, end_date);
        let data = await sequelize.query("select `status`, count(id) as total from orders where(`orders`.createdAt >= :start AND `orders`.createdAt <= :end) group by `status` order by `status`;",
            {
                type: QueryTypes.SELECT,
                replacements: { start, end }
            })

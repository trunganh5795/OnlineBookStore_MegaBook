const { Order, sequelize, Order_detail, amount_discount, Percentage_discount, Predict_cancellation, Notification, Book, Voucher, ShippingAddress, Ward, District, Province } = require('../models');
...
const success_url = process.env.NODE_ENV === 'production' ? process.env.STRIPE_SUCCESS_URL : 'http://localhost:3000/account/orders';
const cancel_url = process.env.NODE_ENV === 'production' ? process.env.STRIPE_CANCEL_URL : 'http://localhost:3000/';
...
const getOrderDetails = async (req, res, next) => {
  try {
    ...
    let whereClause = req.user.role === 'client' ? { user_id: req.user.id } : {};
...
const postCreateOrder = async (req, res, next) => {
  ...
  try {
    ...
    for (let i = 0; i < productList.length; i++) {
      ...
      if (product.instock < productList[i].quantity) return next({ code: 401, msg: 'this product has been out of stock' })
      orderList.push({ book_id: productList[i].bookId, quantity: productList[i].quantity, title: product.title, img: product.img, price: product.price, discount: product.enable_discount ? product.discount : 0 })
      if (productList[i].rcm && productList[i].rcmtype && productList[i].index) {
        trackingList.push({ recombeeid: productList[i].rcm, recommend_type: productList[i].rcmtype, index: productList[i].index, bookid: productList[i].bookId, action: 2 })
      }
      total += product.price * ((100 - (product.enable_discount ? product.discount : 0)) / 100) * productList[i].quantity;
    }
    ...
    await Order_detail.bulkCreate(orderList.map(item => ({
      order_id: order.id,
      book_id: item.book_id,
      quantity: item.quantity,
      discount: item.discount
    })))
    if (paymentMethod === 3 || paymentMethod === 2) { //stripe card return url
...
const webHookStripeEvent = async (req, res, next) => {
  try {
    ...
    let event;
    event = stripe.webhooks.constructEvent(req.body, sig, process.env.WEBHOOK_SECRET); //https://dashboard.stripe.com/test/webhooks/we_1LBbC3C8xirXWd44i3PZOHlY

const { Order, User, Predict_cancellation, Notification, sequelize } = require('../models');
...
const getOrderList = async (req, res, next) => {
    try {
        ...
        let { start_date, end_date, paymentType, status, page, perPage, filters, search, option } = req.query;
        option = +option;
        if (!page) page = 1;
        ...
        if (option) {
            ...
            if (option === 1) {
                whereUserClause.name = sequelize.where(sequelize.fn('LOWER', sequelize.col(`name`)), { [Op.like]: `%${search.toLowerCase()}%` });
            } else {
...
const comfirmOrderBulk = async (req, res, next) => {
    try {
        ...
        ]).then(() => res.status(200).send("OK")).catch(() => next({}))

  ...
  return (fields, { add, remove }) => {
    ...
    return orderList.order_detail?.map((item, index) => (
      <div key={fields[index].key}>
        <Link to={`/product/${item.product.BookId}`}>
          {helpers.reduceProductName(item.product.title, 100)}
        </Link>
        <br />
        ...
        <div style={{ display: 'none' }}>
          <Form.Item
            name={[index, 'bookId']}
            initialValue={item.product.BookId}>
            <TextArea />
          </Form.Item>
        </div>
        <Divider dashed />
...
export default function PostCommentModal({
  ...
  return (
    <Modal
      ...
      footer={null}
      title={<p className="font-size-18px m-b-0">Đánh giá đơn hàng</p>}>
      <Form
        form={form}
        onFinish={(data) => {
        ...
        initialValues={{
          fields: ratingOrder.order_detail?.map((item, index) => index),
        }}>
        <Form.List name="fields">{renderProductList(ratingOrder)}</Form.List>

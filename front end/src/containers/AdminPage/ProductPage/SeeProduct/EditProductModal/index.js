...
function EditProductModal(props) {
  ...
  const onEdit = async (value) => {
    try {
      value.time = [
        value.time[0] ? moment(value.time[0]._d).format('YYYY-MM-DD') : null,
        value.time[1] ? moment(value.time[1]._d).format('YYYY-MM-DD') : null,
      ];
        ...
        <Row gutter={[16, 16]}>
          ...
          <Col span={12}>
            <Form.Item
              name="publisher"
              label="NXB"
              rules={[{ whitespace: true, message: 'Invalid' }]}>
          ...
          {/* <Col span={12}></Col> */}
          <Col span={12}>
            <Image width={300} src={avatar || img} fallback={imgNotFound} />
          ...
          <h2 className="w-100 t-center">
            Chương trình khuyến mãi{' '}
            <Switch
              checked={discountStatus}
              onChange={() => setDiscountStatus((prev) => !prev)}
            />{' '}
          </h2>
          <Col span={12}>
            <Form.Item
              ...
              label="Khuyến mãi"
              rules={
                discountStatus ? [{ required: true, message: 'Bắt buộc' }] : []
              }>
              <InputNumber
                style={{ width: '100%' }}
                step={1}

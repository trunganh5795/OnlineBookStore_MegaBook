...
const rankingTooltipText = {
  total:
    '5 sản phẩm có tổng doanh số cao nhất (tính trên các đơn hàng đã được xác nhận) trong khoảng thời gian đã chọn.',
  quantity:
    '5 sản phẩm bán chạy nhất của Shop theo tổng số lượng sản phẩm đã xác nhận trong khoảng thời gian được chọn.',
...
const segmentedOptions = [
  {
    label: (
      <p>
        Theo doanh số{' '}
        <Tooltip title={rankingTooltipText.total}>
  ...
  },
  {
    label: (
      <p>
        Theo số lượng bán{' '}
        <Tooltip title={rankingTooltipText.quantity}>
          <QuestionCircleOutlined className="icon" />
        </Tooltip>
      </p>
    ),
  ...
  },
  {
    label: (
      <p>
        ...
        <Tooltip title={rankingTooltipText.views}>
          <QuestionCircleOutlined className="icon" />
        </Tooltip>
      </p>
    ),
  ...
  },
  {
    label: (
      <p>
        ...
        <Tooltip title={rankingTooltipText.ratio}>
          <QuestionCircleOutlined className="icon" />
        </Tooltip>
      </p>
    ),
...
export default function Ranking({ time }) {
  ...
  const [columns, setColumns] = useState([
    ...
    },
    {
      ...
      render: (title, record) =>
        record.id ? (
          <Link to={`/product/${record.id}`} target="_blank">
            {helpers.reduceProductName(title, 80)}
          </Link>
        ) : (
          helpers.reduceProductName(title, 80)
        ),
  ...
  useEffect(() => {
    ...
    let getTopSell = async (type) => {
      ...
      if (type === 0) {
        ...
        let result = await statisticApi.getTop5RevenueLocation(
          start_time,
          end_time,
        );
        ...
        let result = await statisticApi.getTop5SellLocation(
          start_time,
          end_time,
        ); //top 5 khu vực nhiều đơn nhất

...
let query = '';
let filterOps = [{ field: 'time', value: [] }];
export default function Voucher() {
  const [voucherDetails, setVoucherDetail] = useState({
    visible: false,
    data: initForm,
  });
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        key: 'discount',
        render: (voucher) => {
          if (voucher.amount.length)
            return (
              <span style={{ color: 'red' }}>
                - {helpers.formatProductPrice(voucher.amount[0].amount)}
              </span>
            );
          else if (voucher.percentage.length)
            return '- ' + voucher.percentage[0].percent + '%';
        },
      },
      {
        title: 'Số lượng',
        ...
        render: (voucher) => (
          <>
            ...
            <Tooltip title="Xóa" placement="left">
              <DeleteOutlined
                onClick={() =>
                  setDelModal({ visible: true, code: voucher.code })
                }
                className="m-r-8 action-btn-product"
                style={{ color: 'red' }}
              />
  ...
  const updateCurrentVoucher = async (data) => {
    try {
      ...
      Object.assign(voucherUpdate, data, {
  ...
  const getAllVoucher = async (
    ...
    if (option === 0) {
      ...
      const response = await adminApi.getVoucherListBy(
        value,
        option,
        page,
        perPage,
  ...
  return (
    <div className="m-lr-10 m-t-10">
      ...
      <VoucherModal
        ...
        title={title.title}
        handleData={
          title.type === 'edit' ? updateCurrentVoucher : addNewVoucher
        }

...
export default function BestBook() {
  ...
  return (
    <Col span={24}>
      <Row
        gutter={[10, 10]}
        className="p-30 m-b-32 bg-white box-sha-home bor-rad-8 best-book"
        ...
        <Col span={24} md={16}>
          ...
          <h3 className="border-bottom-line p-b-5 m-b-10 t-color-white">
            {bestBook.author}
          </h3>
          ...
          <Row>
            {/* <Col md={8}> */}
            <div
              className="price d-flex align-i-center"
              style={{ flexGrow: 1 }}>
              <h1 className="t-color-white font-weight-700 p-tb-8 m-r-10">
                {helpers.formatProductPrice(
                  (bestBook.price *
                    (100 - (bestBook.discount ? bestBook.discount : 0))) /
                    100,
                )}
              </h1>
              {bestBook.discount > 0 &&
                bestBook.enable_discount &&
                moment(bestBook.start_time).isSameOrAfter(
                  moment().format('YYYY-MM-DD'),
                ) &&
                moment(bestBook.end_time).isBefore(
                  moment().format('YYYY-MM-DD'),
                ) && (
                  <>
                    ...
                    <span className="m-l-8 discount-badge">-{10}%</span>
                  </>
                )}
            </div>
            <Button

  ...
  return (
    <>
      {isAuth != null && (
        <Col
          span={24}
          className="m-b-32 bg-white box-sha-home bor-rad-8 trending-books">
          <Row className="p-16">
            ...
            <Col span={24}>
              <Tabs tabPosition="left">
                {trendingList.recomms &&
                  trendingList.recomms.map((item) => {
                    let Itemindex = carts.findIndex(
                      (element) => element.bookId === item.id,
                    );
                    if (Itemindex !== -1) {
                    ...
                    return (
                      <TabPane
                        ...
                        <Link to={`product/${item.id}`}>
                          <h1 className="t-color-black title-hover">
                            {helpers.reduceProductName(item.values.title, 80)}
                          </h1>
                        </Link>
                        <h3 className="border-bottom-line p-b-5 m-b-10 t-color-black">
                          {item.values.author}
                        </h3>
                        {item.values.description ? (
                          <p className="border-bottom-dashed p-b-10 t-color-black">
                            {item.values.description}{' '}
                          </p>
                        ) : (
                          <p className="border-bottom-dashed p-b-10 t-color-black">
                        ...
                        <Row>
                              ...
                              <h1 className="price t-color-black font-weight-700 p-tb-8 m-r-10">
                                ...
                                {1 === 0
                                  ...
                                  : helpers.formatProductPrice(
                                      (item.values.price *
                                        (100 - item.values.discount)) /
                                        100,
                                    )}
                              </h1>
                              {item.values.discount ? (
                                <>
                                  {' '}
                                  ...
                                  <span className="m-l-8 discount-badge">
                                    -{10}%
                                  </span>{' '}
                                </>
                              ) : (
                          ...
                          <Col span={24}>
                            <Button
                              ...
                              disabled={
                                item.values.instock <= 0 ? true : false
                              }>
                              Thêm vào giỏ
                            </Button>
                          </Col>

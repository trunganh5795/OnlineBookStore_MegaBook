...
export default function BarChart() {
    ...
    useEffect(() => {
        ...
        let getHistogramData = async () => {
            ...
            let result = await statisticApi.getHistogramData(rangeTime[0], rangeTime[1])
            let dataChart = result.data.buckets.map(item => {
                return ({
                    x: ((item.from ? (item.from / 1000 === 1000 ? 2100 : item.from / 1000) : 0) + (item.to ? item.to / 1000 : 0)) / 2,
                    y: item.doc_count
                })
            })
    ...
    return (
        <div>
            <div className='pos-relative'>
                ...
                >
                    <Option value={'now/d,now/d'}>Hôm nay ({moment().format('DD/MM/YYYY')})</Option>
                    <Option value={'now-7d/d,now/d'}>7 Ngày ({moment().subtract(7, 'd').format('DD/MM/YYYY')} - {moment().format('DD/MM/YYYY')})</Option>
                    <Option value={'now-30d/d,now/d'}>30 Ngày ({moment().subtract(30, 'd').format('DD/MM/YYYY')} - {moment().format('DD/MM/YYYY')})</Option>

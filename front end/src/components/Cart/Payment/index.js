...
function CartPayment(props) {
  ...
  const totalDiscount = carts.reduce(
    (totalDiscount, product) => totalDiscount + ((product.price * product.discount) / 100) * product.amount,
    0);
  useEffect(() => {
    ...
    let items = voucherList?.map(item => ({
      ...
      label: (
        <Row gutter={8} style={{ width: '150px' }}>
          ...
          <Col span={12}>
            <b>{item.code}</b>
            <div><b style={{ color: 'red' }}>- {item.percentage[0]?.percent || helpers.formatProductPrice(item.amount[0]?.amount)} {item.percentage[0] ? " %" : ""}</b></div>
          </Col>
          <i className='voucher-desc'>Áp dụng cho {helpers.convertCategoryIdToString(item.apply)}</i>
        </Row>)
    }))
    ...
    {isCheckout ? (
      <>
        <div className="d-flex justify-content-between m-b-6">
          ...
          <b style={{ color: 'red' }}>{voucherDiscountValue !== 0 && `- ${helpers.formatProductPrice(voucherDiscountValue)}`} </b>
        </div>
        <div className="d-flex justify-content-between">
          ...
          <b style={{ color: 'red', fontSize: 20 }}>
            {transportFee === null ? "--" : helpers.formatProductPrice(transportFee)}
          </b>
        </div>
      </>
    ) : ""}

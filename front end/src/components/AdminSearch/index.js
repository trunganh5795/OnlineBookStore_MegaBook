...
export default function AdminSearch({ options, onSearch, selectedOption }) {
    ...
    return (
        <Input.Group className='m-t-10'>
            <Select defaultValue={selectedOption} style={{ width: 180 }} size='large' onChange={(value) => setOption(value)}>
                {options.map((item, index) => (<Option value={item.id} key={index}>{item.text}</Option>))}
            </Select>
            <Input.Search

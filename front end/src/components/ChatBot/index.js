...
const showMessages = (mess) => {
  return mess.map((item, index) => {
    if (item.type === 'bot') {
      ...
      return (
        <div className="message-box-holder" key={index}>
          <div className="message-box">{item.content}</div>
        </div>
      );
    } else {
      return (
...
export default function ChatBox() {
  ...
  const sendMessage = async (text) => {
    try {
      ...
      setMessages((prev) => [
        ...prev.slice(0, prev.length - 1),
        { type: 'bot', content: 'Có lỗi xảy ra, hãy liên hệ nhân viên hỗ trợ' },
      ]);
    }
  };
  useEffect(() => {
            ...
            <div className="">
              <Form
                ...
                <Form.Item
                  ...
                  <Input
                    addonAfter={
                      <SendOutlined onClick={form.submit} autoFocus />
                    }
                    ref={inputText}
                  />
                </Form.Item>
